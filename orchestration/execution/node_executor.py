...
class NodeExecutionContext(BaseModel):
    ...
    run_dir: str
    # A bounded, framework-built memory view.  Executors may read it but have
    # no store handle and therefore cannot mutate long-term memory directly.
    memory_capsule: dict[str, Any] = Field(default_factory=dict)
